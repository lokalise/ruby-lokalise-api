...
RSpec.describe Lokalise::Client do
  ...
  let(:queued_process_id) { '6347ddfc2ed1bb855e670fd7ad16ce8552333af8' }
  ...
  specify '#queued_process with process type' do
    process = VCR.use_cassette('queued_process_detailed') do
      test_client.queued_process project_id, process_id, 'file-import'
    end
  ...
  describe 'process status check' do
    specify '#reload_data' do
      queued_process = VCR.use_cassette('upload_file_queued_status') do
    ...
    specify '#reload_data with detailed' do
      ...
      reloaded_process = VCR.use_cassette('queued_process_detailed') do
        process.reload_data
      end
